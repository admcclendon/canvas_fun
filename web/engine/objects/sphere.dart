...
class Sphere extends Object3D
{
   ...
   * 
   * The total number of verticies is, (phiSteps - 2)*thetaSteps + 2. This is 
   * because the top and bottom of the sphere only needs 1 vertex. Each section
   * between uses thetaSteps vertices.
   * 
   * The total number of faces is, 2 * thetaSteps * (phiSteps - 2). This is because
   * the top and bottom of the sphere only need thetaSteps faces which becomes
   * 2 * thetaSteps. Then there is (phiSteps - 3) levels left with 2 * thetaSteps faces.
   * Adding those terms and simplifying results in 2 * thetaSteps * (phiSteps - 2).
   */
  Sphere(this.radius, int thetaSteps, int phiSteps, Point3D position) : super((phiSteps - 2)*thetaSteps + 2, 2*thetaSteps*(phiSteps - 2), position)
  {
    ...
    for (int i = 0; i < phiSteps - 1; i++)
    {
      for (int j = 0; j < thetaSteps; j++)
      {
        ...
        {
          this.faces[(2*i - 1)*thetaSteps + j] = new Face([
            this.trans_verts[(i-1)*thetaSteps+j+1], 
            this.trans_verts[(phiSteps - 2)*thetaSteps + 1], 
            this.trans_verts[(i-1)*thetaSteps+1+(j+1)%thetaSteps]], "rgb(0, 0, 255)");
        }
        else
        {
          this.faces[(2*i - 1)*thetaSteps + j*2] = new Face([
            this.trans_verts[(i-1)*thetaSteps+j+1],
            this.trans_verts[i*thetaSteps+j+1],
            this.trans_verts[(i-1)*thetaSteps+1+(j+1)%thetaSteps]], "rgb(0, 0, 255)");
